...
function FundraiserSection({ f }: { f: FundraiserProps }) {
  ...
  return (
    <div
      className="mb-6 pb-6 border-b border-forest/15"
      data-tina-field={f.tinaFields?.title}
    >
      <div className="flex items-center justify-between gap-3 flex-wrap mb-2">
        <span
          className="text-xs font-semibold text-forest/70 uppercase tracking-wider"
          data-tina-field={f.tinaFields?.title}
        >
        ...
        {f.deadline && days !== null && (
          <span
            className="text-xs text-dark/40 whitespace-nowrap"
            data-tina-field={f.tinaFields?.deadline}
          >
      ...
      <div className="relative h-1.5 bg-dark/10 rounded-full overflow-hidden mb-1.5">
        <div
          className="h-full bg-forest/60 rounded-full transition-all duration-700"
          style={{ width: `${percent}%` }}
        />
      </div>
...
export default function DonationBox({
  ...
  return (
    <div
      className="bg-light-green rounded-2xl p-6 sm:p-8"
    ...
    >
      ...
      <div
        className="flex-none text-center m-4"
        data-tina-field={tinaFields?.qrCodeImage}
      >
      ...
      <dl className="grid grid-cols-[max-content_1fr] items-baseline gap-x-4 gap-y-1.5 text-sm mb-4">
        <dt className="text-dark/60 shrink-0">Č. účtu:</dt>
        <dd
          className="font-mono font-bold min-w-0 break-all"
          data-tina-field={tinaFields?.accountNumber}
        >
          {accountNumber}
        </dd>
        ...
        {variableSymbol && (
          <>
            <dt className="text-dark/60 shrink-0">Variabilní symbol:</dt>
            <dd
              className="font-mono font-bold min-w-0"
              data-tina-field={tinaFields?.variableSymbol}
            >
              {variableSymbol}
            </dd>
          </>
        ...
        {transferMessage && (
          <>
            <dt className="text-dark/60 shrink-0">Zpráva:</dt>
            <dd
              className="min-w-0"
              data-tina-field={tinaFields?.transferMessage}
            >
              {transferMessage}
            </dd>
          </>
        ...
        {iban && (
          <>
            <dt className="text-dark/60 shrink-0">IBAN:</dt>
            <dd
              className="font-mono font-bold min-w-0 break-all"
              data-tina-field={tinaFields?.iban}
            >
              {iban}
            </dd>
          </>
        ...
        {swift && (
          <>
            <dt className="text-dark/60 shrink-0">SWIFT:</dt>
            <dd
              className="font-mono font-bold min-w-0"
              data-tina-field={tinaFields?.swift}
            >
              {swift}
            </dd>
          </>
